...
if Config.BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query == "hellbot_help":
            ...
            result = builder.article(
                "@smoker_ki_janta",
                text="""**Hey! This is [☠Smøkèr ʙᴏᴛ☠](https://t.me/smoker_ki_janta) \nYou can know more about me from the links given below 👇**""",
                buttons=[
                    [
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid or event.query.user_id in Config.SUDO_USERS:
            ...
            await event.edit(f"**⚜️ ☠Smøkèr ʙᴏᴛ☠ Mêñû Prõvîdêr ìs ñôw Çlösëd ⚜️**\n\n**Bot Of :**  {hell_mention}\n\n        [©️ SmøkèrBøt ™️]({chnl_link})", buttons=veriler, link_preview=False)
        else:
            reply_pop_up_alert = "Hoo gya aapka. Kabse tapar tapar dabae jaa rhe h. Khudka bna lo na agr chaiye to. © ☠Smøkèr ʙᴏᴛ☠ ™"
            await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
